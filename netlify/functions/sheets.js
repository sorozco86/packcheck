...
async function searchSeguridad(modelNo) {
  ...
  // Filtrar filas con modelo vacío antes de buscar
  const validRows = rows.slice(1).filter(r => (r[COL_MODEL] || '').trim().length >= 3);
  console.log('Filas válidas (con modelo):', validRows.length, 'de', rows.length - 1, 'totales');

  // Búsqueda exacta primero
  for (const row of validRows) {
    const cellValue = row[COL_MODEL].trim().toUpperCase();
    if (cellValue === modelNo) {
  ...
  if (matches.length === 0) {
    ...
    for (const row of validRows) {
      ...
      if (cell.includes(modelNo) || modelNo.includes(cell)) {
        partial.push(rowToObj(headers, row));
      }
    }
    if (partial.length > 0) return { found: true, count: partial.length, data: partial, partial: true };

    return { found: false, message: `Modelo "${modelNo}" no encontrado en checklist` };
  }

  // Enriquecer cada fila indicando si es cargador o producto
  const enriched = matches.map(row => {
    const freq = (row['Frecuencia'] || '').toLowerCase();
    const voltaje = (row['Voltaje'] || '').toLowerCase();
    const producto = (row['Producto'] || '').toLowerCase();
    const esCargador =
      freq.includes('cargador') ||
      freq.includes('charger') ||
      voltaje.includes('salida') ||
      voltaje.includes('entrada') ||
      voltaje.includes('output') ||
      producto.includes('cargador') ||
      producto.includes('charger');
    return { ...row, _tipo: esCargador ? 'CARGADOR' : 'PRODUCTO' };
  });

  console.log('Matches para', modelNo + ':', enriched.map(r => r._tipo + ' | ' + r['Voltaje']).join(' / '));

  return { found: true, count: enriched.length, data: enriched };
}
