...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (seguridadResult.found && seguridadResult.data && seguridadResult.data[0]) {
      // Buscar columna Enchufe en los datos
      const row = seguridadResult.data[0];
      claseEnchufe = row['Enchufe'] || row['enchufe'] || row['ENCHUFE'] || '';
      // Buscar por cualquier clave que contenga "enchufe"
      if (!claseEnchufe) {
        for (const [k, v] of Object.entries(row)) {
          if (k.toLowerCase().includes('enchufe') && v) { claseEnchufe = v; break; }
        }
      }
